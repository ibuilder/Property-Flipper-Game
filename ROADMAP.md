...
- ~~**Persist Scout's cooldowns.**~~ **Done**, save v16. An older save resumes
  with the coach having said nothing rather than with an invented history.
- ~~**Retire the legacy colour aliases.**~~ **Mostly done.** What actually
  remained of the dark-only assumption was six hardcoded hover and focus
  backgrounds, now derived from the ground. The `--good/--warn/--bad` names
  themselves flip correctly per theme and stay.
- ~~**Permits and the inspector queue.**~~ **Done**, with a deliberate
