...
**What already exists.** The delivered art is in the game and is rendered in
`art-sheet.svg` (the houses, every archetype in every condition) and
`ui-art-sheet.svg` (icons, Scout, the press set) alongside this file, at the
exact size it appears on screen. Both are regenerated by the test suite, so
they cannot go stale while you are quoting against them.
...
rival, inspector — which were not asked for and are welcome. All four are
placed, each on the panel that reports what that character does: the inspector
