...
4. **A before/after pair** from the track record, with the P&L waterfall
   beneath it.
5. **The board**, in the coloured style, with a job running so Scout is on
   site and a for-sale board on a listing. This is the one that reads as a game
   rather than as a spreadsheet, so it is worth two shots -- the second in the
   line style, which is the same town drawn as a survey plat.
6. **A deal card**, which is the artefact a player takes away and the cheapest
   advertising this will ever get. `docs/design/deal-card.svg` is a rendered
   example.

Capture at 1280 × 800 in the browser build so they match the embed.

`docs/images/board-colour.svg` and `board-line.svg` are generated by the test
run and can be used directly for 5 rather than captured by hand.
